...
class TchController extends Controller {
    ...
    public function result(){
        ...
        $ado_id =  $ado -> where('test_id = '.$_GET["id"])->  getField('ado_id',true);                  //将练习id和练习对应的ado进行匹配
        for($i = 0; $i < count($ado_id); $i++){                                                         //将ado_id与ado_name进行匹配
            $ado_name[] =  $ado -> where('ado_id = "'.$ado_id[$i].'"') ->  getField('ado_name');                
        }
        for($i = 1; $i <= count($ado_id); $i++){                                                        //获得统计图数据
            $map['test_id'] = $_GET["id"];
            $map['ado_id'] = $ado_id[$i-1];      
            $temp['x'] = $ado_name[$i-1];
